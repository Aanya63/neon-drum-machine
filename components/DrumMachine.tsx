...
export default function DrumMachine() {
  ...
  // --- Mutable refs the scheduler reads (kept in sync every render) ---
  ...
  const start = useCallback(() => {
    ...
    setIsPlaying(true);
    stepRef.current = 0;
    nextTimeRef.current = c.currentTime + 0.1;
    queueRef.current = [];
    ...
    <div className="w-full max-w-5xl">
      ...
      <header className="mb-8 flex flex-col items-center text-center">
        ...
        <p className="mt-2 text-sm text-white/50">
          Tap a pad or use your keyboard · build a loop in the sequencer · hit play
        </p>
      </header>
      ...
      {/* Sequencer */}
      <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-6 backdrop-blur">
        {/* Transport */}
      ...
      <footer className="mt-6 text-center text-xs text-white/30">
        Web Audio API · Next.js · Tailwind · 100% client-side. Keys: 1-4 / Q-R / A-F / Z-V · Space = play/stop
      </footer>
    </div>
  );
}
